	 ...
	 static int pawnBoard[][]=
		    ...
		    public static int rateMoveablitly(int listLength, int depth, int material) {
		        ...
		        if (listLength==0) {//current side is in checkmate or stalemate
		            if (!albchess.kingSafe()) {//if checkmate
		        ...
		        for (int i=0;i<64;i++) {
		            switch (albchess.chessBoard[i/8][i%8]) {
		                ...
		                case "Q": counter+=queenBoard[i/8][i%8];
		                    break;
		                case "A": if (material>=1750) {counter+=kingMidBoard[i/8][i%8]; counter+=albchess.posibleA(albchess.kingPositionC).length()*10;} else
		                {counter+=kingEndBoard[i/8][i%8]; counter+=albchess.posibleA(albchess.kingPositionC).length()*30;}
		                    break;
		            }
		        }
		        return counter;
